...
function collectUploads(options) {
  return request.getAsync({ url: options.host + allUploadsPath(), json: true, headers: { "x-api-key": options.apiKey } }).then(function(response) {
    return response.body.uploads;
  }).map(function(upload) {
    return request.getAsync({ url: options.host + uploadPath(upload.id), json: true, headers: { "x-api-key": options.apiKey } });
...
function collectPosts(options) {
  return request.getAsync({ url: options.host + allPostsPath(), json: true, headers: { "x-api-key": options.apiKey } }).then(function(response) {
    return response.body.posts;
  }).map(function(post) {
    return request.getAsync({ url: options.host + postPath(post.id), json: true, headers: { "x-api-key": options.apiKey } });
  }).map(function(response) {
    return response.body;
  });
}
...
function collectPages(options) {
  return request.getAsync({ url: options.host + allPagesPath(), json: true, headers: { "x-api-key": options.apiKey } }).then(function(response) {
    return response.body.pages;
  }).map(function(page) {
    return request.getAsync({ url: options.host + pagePath(page.id), json: true, headers: { "x-api-key": options.apiKey } });
  }).map(function(response) {
    return response.body;
  });
}
...
function collectMicro(options) {
  return request.getAsync({ url: options.host + allMicroPath(), json: true, headers: { "x-api-key": options.apiKey } }).then(function(response) {
    return response.body.micro_posts;
  }).map(function(micro) {
    return request.getAsync({ url: options.host + microPath(micro.id), json: true, headers: { "x-api-key": options.apiKey } });
  }).map(function(response) {
    return response.body;
  });
}
