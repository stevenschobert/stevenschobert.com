...
helpers.printMonth = function printMonth(month, format) {
  var fmt = format || "MMM";
  return moment().month(month).format(fmt);
};
...
helpers.printDateTime = function printDate(date, format) {
  return helpers.printDate(date, "MMM D, YYYY h:mm a");
};
